...
> The user has asked you to install the "factor-backtest-skill" skill.
...
2. Check if `$HOME/.claude/skills/factor-backtest-skill/` already exists
   - If YES: Ask the user "Skill 'factor-backtest-skill' already installed. Overwrite? (y/n)"
   - If NO: Proceed

## Security Declaration

This installation will:
- CREATE directory: `$HOME/.claude/skills/factor-backtest-skill/`
- CREATE file: `$HOME/.claude/skills/factor-backtest-skill/SKILL.md`
...
Run: `mkdir -p $HOME/.claude/skills/factor-backtest-skill`

### Step 2: Write SKILL.md

Write the following content to `$HOME/.claude/skills/factor-backtest-skill/SKILL.md`:
...
Read `$HOME/.claude/skills/factor-backtest-skill/SKILL.md` and confirm:
...
Tell the user:
"factor-backtest-skill installed successfully!
- Location: ~/.claude/skills/factor-backtest-skill/
- Trigger: 安全、高效地批量回测 8 个 Alpha 表达式，内置验证、熔断和异常处理机制。
- To uninstall: delete the ~/.claude/skills/factor-backtest-skill/ directory"

## Rollback

If ANY step fails:
1. Delete the `$HOME/.claude/skills/factor-backtest-skill/` directory if it was created
