...
前端展示的信息必须同时满足两个条件：**给人看**，且**人方便看**。给机器看的东西不该出现在前端。

接口可以全量返回（省请求），但前端必须筛选。判断标准：这个字段能不能帮助人做决定？

- **给人看的**：结论、证据、指标、原因、时间——帮助人回答「这是什么？行不行？怎么办？」
- **给机器看的**：哈希（sha256、代码/数据快照 hash）、run id、服务器文件路径、内部枚举原文、对象关系 ID——这些是程序对账用的，留在数据库和接口里，真要查用 CLI/SQL

真实案例（2026-08，因子详情页）：后端返回全部字段，前端照单全收，页面上平铺三个 64 位哈希、两列哈希列表、每条时间线挂 ID+双哈希、产物列表带服务器绝对路径。用户评价："产品设计简直是一坨"。服务器路径上前端除了难看还是安全问题（暴露目录结构）。

配套做法：
- **页面结构按人想回答的问题排序**：结论 → 这是什么 → 证据 → 背景。不要按数据库表结构排
- **机器枚举必须翻译**：`candidate` → 候选中、`active` → 进行中、`succeeded` → 成功。原始枚举串不上页面
- **中英双语**：英文术语首次出现配中文，如「RankIC（秩相关）均值」「FDR q 值（多重检验修正）」；代码参数 key 翻成中文、括号保留英文：`防御资产 (defensive_instrument)`
- **状态无事不报**：正常状态不占用视觉，只有异常才出标记
- **一个页面一个权威结论**：结论徽章带颜色，其余指标回到单色数字；不要每个指标各自染红绿重复表态、互相矛盾
...
写完页面自查：
0. 页面上每个字段都过了「这能帮人做决定吗」这一关？有没有哈希/ID/路径/枚举原文混进来？
