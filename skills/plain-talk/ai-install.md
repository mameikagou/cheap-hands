...
任何术语、缩写、项目名第一次出现时，必须当场用一句话解释它是什么、量什么或干什么——只解释用途，不解释原理，不展开历史。第二次出现就不用再解释。

但注意反面：不要为了避免术语而把术语删掉。术语是凝练的知识，一个术语背后是一整套认知；全程只用大白话，对方听完什么都记不住，也没法拿这些词去查资料、跟别人沟通。正确做法是把每个术语当成一次教学：术语 + 人话解释 + 它量/指什么，成对出现。目标是让对方学会这些术语，而不是让回复里没有术语。

- 改前：支持 OAuth/OIDC 登录、两步验证、Master/Worker 架构。
...
1. 这里的每个名词第一次出现时，我不查能懂吗？→ 不能就补一句用途解释。
