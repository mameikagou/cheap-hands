...
> The user has asked you to install the "suization" skill.
...
2. Check if `$HOME/.claude/skills/suization/` already exists
   - If YES: Ask the user "Skill 'suization' already installed. Overwrite? (y/n)"
   - If NO: Proceed

## Security Declaration

This installation will:
- CREATE directory: `$HOME/.claude/skills/suization/`
- CREATE file: `$HOME/.claude/skills/suization/SKILL.md`
...
Run: `mkdir -p $HOME/.claude/skills/suization`

### Step 2: Write SKILL.md

Write the following content to `$HOME/.claude/skills/suization/SKILL.md`:

```
---
name: suization
description: Suization（酥化）项目叙事：把零散的真实项目经历改写成强势、具体、可深挖的技术链路，并生成简历要点、项目详述和面试追问稿。用户说“酥化”或“Suization”、要求包装项目、补技术链路、写项目经历、强化 owner 感、准备项目面试，或提供代码、文档、工作记录让 Codex 提炼项目叙事时使用。
---
...
Read `$HOME/.claude/skills/suization/SKILL.md` and confirm:
...
Tell the user:
"suization installed successfully!
- Location: ~/.claude/skills/suization/
- Trigger: Suization（酥化）项目叙事：把零散的真实项目经历改写成强势、具体、可深挖的技术链路，并生成简历要点、项目详述和面试追问稿。用户说“酥化”或“Suization”、要求包装项目、补技术链路、写项目经历、强化 owner 感、准备项目面
- To uninstall: delete the ~/.claude/skills/suization/ directory"

## Rollback

If ANY step fails:
1. Delete the `$HOME/.claude/skills/suization/` directory if it was created
