...
> The user has asked you to install the "suhua" skill.
...
2. Check if `$HOME/.claude/skills/suhua/` already exists
   - If YES: Ask the user "Skill 'suhua' already installed. Overwrite? (y/n)"
   - If NO: Proceed

## Security Declaration

This installation will:
- CREATE directory: `$HOME/.claude/skills/suhua/`
- CREATE file: `$HOME/.claude/skills/suhua/SKILL.md`
...
Run: `mkdir -p $HOME/.claude/skills/suhua`

### Step 2: Write SKILL.md

Write the following content to `$HOME/.claude/skills/suhua/SKILL.md`:

```
---
name: suhua
description: “酥化”项目叙事：把零散的真实项目经历改写成强势、具体、可深挖的技术链路，并生成简历要点、项目详述和面试追问稿。用户说“酥化”、要求包装项目、补技术链路、写项目经历、强化 owner 感、准备项目面试，或提供代码、文档、工作记录让 Codex 提炼项目叙事时使用。
---
...
Read `$HOME/.claude/skills/suhua/SKILL.md` and confirm:
...
Tell the user:
"suhua installed successfully!
- Location: ~/.claude/skills/suhua/
- Trigger: “酥化”项目叙事：把零散的真实项目经历改写成强势、具体、可深挖的技术链路，并生成简历要点、项目详述和面试追问稿。用户说“酥化”、要求包装项目、补技术链路、写项目经历、强化 owner 感、准备项目面试，或提供代码、文档、工作记录让 Code
- To uninstall: delete the ~/.claude/skills/suhua/ directory"

## Rollback

If ANY step fails:
1. Delete the `$HOME/.claude/skills/suhua/` directory if it was created
